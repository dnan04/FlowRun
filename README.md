...
- **执行防护**：支持 5 段 Cron 执行时间窗口、基于 `PENDING/RUNNING` 记录的任务级重复触发拦截，以及任务中心执行按钮 5 秒防重复点击。
...
重复触发窗口单位为分钟，留空表示不限制；填写后仅拦截窗口期内同一任务仍处于 `PENDING/RUNNING` 的重复提交，`SUCCEEDED/FAILED` 不会被拦截。任务中心前端还会对同一任务的执行按钮进行 5 秒防重复点击。
