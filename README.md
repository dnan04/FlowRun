...
- **管理后台**：管理员维护任务目录、任务配置、执行方式、授权人员、失败联系人、测试状态、发布状态和任务中心展示状态。
...
- **执行防护**：支持 5 段 Cron 执行时间窗口、基于 `PENDING/RUNNING` 记录的任务级重复触发拦截，以及任务中心执行按钮 3 秒防重复点击。
...
- `后置方法`：DS 流程成功后调用 PG 执行库中的 PostgreSQL 查询方法，可不填；填写时必须是一条包含 `:job_instance_name` 的 `SELECT` 语句。
...
重复触发窗口单位为分钟，留空表示不限制；填写后仅拦截窗口期内同一任务仍处于 `PENDING/RUNNING` 的重复提交，`SUCCEEDED/FAILED` 不会被拦截。任务中心前端还会对同一任务的执行按钮进行 3 秒防重复点击。
